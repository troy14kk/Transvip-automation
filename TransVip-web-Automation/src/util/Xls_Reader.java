...
public class Xls_Reader {
	...
	public boolean setCellData(String sheetName,String colName,int rowNum, String data,String url){
		...
		//HSSFHyperlink link = (HSSFHyperlink) createHelper.createHyperlink(XSSFHyperlink.LINK_FILE);
	   // link.setAddress(url);
  ...
  // find whether sheets exists	
	...
	public boolean setCellDatacol(String sheetName,String colName,int rowNum, String data,String testname){
		try{
		fis = new FileInputStream(path); 
		workbook = new HSSFWorkbook(fis);
		...
		for(int rNum=1;rNum<=getRowCount("Test Data");rNum++){
			if(getCellData("Test Data", 0, rNum).equals(testname)){
				testStartRowNum=rNum;
				break;
			}
		}
		row=sheet.getRow(testStartRowNum);
		if(row.getCell(0).getStringCellValue().trim().equalsIgnoreCase("Runmode"))
		{
